...
async def analyze_chart_with_claude(image_bytes: bytes, mime_type: str) -> str:
    """Kirim gambar ke Claude API dan return hasil analisa."""
    if not ANTHROPIC_KEY:
        return "❌ `ANTHROPIC_API_KEY` belum diset di environment variable Railway."
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(
            ...
            if resp.status != 200:
                err = await resp.text()
                raise RuntimeError(f"Claude API error {resp.status}: {err[:300]}")
...
async def analyzechart_cmd(
    ...
    try:
        ...
        prompt_extra = f"\n\nCatatan dari user: {catatan}" if catatan else ""
        global ANALYZE_PROMPT
        full_prompt = ANALYZE_PROMPT + prompt_extra

        # Override prompt sementara kalau ada catatan
        payload_prompt = full_prompt

        result = await analyze_chart_with_claude(image_bytes, clean_mime)
        ...
        embed.set_footer(
            text=f"Dianalisa oleh Claude · Upload by {interaction.user.display_name}"
        )
